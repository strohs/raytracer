...
impl Perlin {
    ...
    pub fn new() -> Self {
        let mut rand_vecs = [Vec3::default(); POINT_COUNT];
        for i in 0..POINT_COUNT {
            rand_vecs[i] = Vec3::random_range(-1.0, 1.0).unit_vector();
        }
    ...
    /// it takes a 3D point as input `p` and always returns the same "randomish number". Nearby
    /// points return similar numbers. Another important part of Perlin noise is that it be
    /// simple and fast, so it’s usually done as a hack...
    pub fn perlin_noise(&self, p: &Point3) -> f64 {
        let u = p.x() - p.x().floor();
        let v = p.y() - p.y().floor();
        let w = p.z() - p.z().floor();
    ...
    /// Returns a "shuffled" array of integers in the range of 0..POINT_COUNT
